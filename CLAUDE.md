...
| `js/render.js` | `afterChange()` (save + repaint), the capacity rules editor, the totals |
| `js/render-calendar.js`, `js/render-board.js`, `js/render-flags.js` | calendar outputs, roster and cards (and the board bar: This plan / Later / Done, Cards / Table), scoped flags dialog |
| `js/render-table.js` | the table view (sortable, totals, When and Lands columns, same drag contract as the cards) and the Later and Done lists |
| `js/timeline.js` | pure: `spanOf` (a deliverable's sprints), `splitExact`, `spread` (a share's points across its sprints), `landing` (the estimated completion date) |
| `js/quarters.js` | pure: fiscal quarters (`quarterOf`, `planQuarters`, `quartersAround`, `quarterStartMonday`, `monthsIn`) |
| `js/render-map.js`, `js/map-edit.js` | the Map view (months, sprints, people with holidays and vacations and per-sprint load, deliverable bars with landing diamonds) and its editing (drag a vacation, drag or resize a bar, arrow keys) |
| `js/planning.js` | validated priority, progress and preset colors; stable priority sorting, separate from capacity |
...
- `settings.fiscalStart` (1 to 12, default 10) is the month the fiscal year starts: it names quarters (Q1 FY27) and picks the default start, never moves a date. New plans inherit it from the open plan.
- `deliverables[].window` is `{ from, to }` (1-based sprints, inclusive) or `null` for the whole plan.
- `doc` is `{ title, settings, daysOff, people, deliverables, backlog }`. A share is `deliverables[].members[] = { person, pct }` (or legacy fixed `{ person, points }`). Deliverables also carry `priority` (`p1` through `p6`), `progress`, and `boxColor` (empty follows priority). Badge colors are fixed. Old priority names migrate to P1/P2/P3/P6, and legacy `priorityColor` becomes the box override. `backlog[]` is a deliverable plus `when: 'later' | 'done'`, members kept. Completion is derived from `when: 'done'`; its stored `progress` is the prior active state for restoration.
...
- **Cards and table share the board.** Flags open in their own dialog. The view-only `ui.personFilter` filters Cards, Table, Later and Done while totals remain whole-plan. Rows keep `data-drop` and the share chips keep `data-drag`, so drag, carry and fixes work unchanged; a row's `id` is `d-<id>` like a card's, because only one view renders at a time.
...
- **A share is a percentage of the person's time in its deliverable's sprints.** `analyze()` splits each person's `cap` across sprints as exact fractions (`sprintCap`, `plannedSprints`; whole points gave the first half of a plan a point more than the second), a share's base is its span's points (`winCap`: the whole `cap` for a whole-plan share, so plans with no windows read exactly as before), and its points land in its sprints in proportion (`spread`). Over-booking is per sprint (`overSprints`, `peak`, `overPts`) as well as across the plan; spread is deliverables at once (`concurrent`), not in total.
- **Changing a deliverable's sprints keeps each person's points by default** (`changeWindow`, via `pctForPoints` against the new span, then `keepOthers`), so the same work in fewer sprints asks for a bigger share of their time; the popover previews that and the landing date. `keepPoints: false` keeps the percentages instead.
- **Landing dates are an estimate at the deliverable's own pace** (`landing` in timeline.js): the working day its running total reaches the estimate inside its sprints, else the span's average pace carried on past it (and past the plan, `afterPlan`), else 'far' past two years. Working days are weekdays up to `daysPerWeek`; holidays inside the extrapolation are not taken out. The map follows a late landing up to a quarter past the plan.
...
- **One status per deliverable.** `deliverableStatus()` in flags.js feeds the cards, the Markdown report and the tables, and "At risk" (an over-booked or open-role member) beats a green "Staffed". The over-booking flag carries `target.also`, the cards that person is on, so their scoped details include it.
...
- **The sticky roster lives in `.workspace-main`, its own grid with the board.** A sticky grid item is bounded by the grid container, not its area. Keep it within the board's container.
